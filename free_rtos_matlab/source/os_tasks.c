...
/*
 * Defines, wether the EDFVD adjustment is used.
 */
...
typedef struct tskTaskControlBlock
{
    ...
    char                pcTaskName[ configMAX_TASK_NAME_LEN ];/*< Descriptive name given to the task when created.  Facilitates debugging only. */ /*lint !e971 Unqualified char types are allowed for strings and single characters only. */
        ...
        UBaseType_t virtualDeadline;
        UBaseType_t VD_is_set;
        UBaseType_t c_low;
        UBaseType_t c_high;
        UBaseType_t xi;
    #endif
...
#define prvAddTaskToReadyList( pxTCB )                                                              \
  { listSET_LIST_ITEM_VALUE( &( ( pxTCB )->xGenericListItem ),task_set_DEADLINE(pxTCB) );            \
    vListInsert( &(xReadyTasksListEDFVD), &( ( pxTCB )->xGenericListItem ) );} // inserts descending i.e. lowest item first
#else
#define prvAddTaskToReadyList( pxTCB )                                                              \
...
//                                    if(pxTCB->xi==1){ // if its a high critical task
//                                    pxTCB->virtualDeadline=pxTCB->Deadline*x;
//                                    }
//                                    else{
//                                    // low critical tasks don't have a virtual deadline
//                                    }
//                                }
//                                it++;
//                            }
...
BaseType_t xReturn;
    ...
    unsigned int para_idle_task[4];
    para_idle_task[0]=UINT32_MAX;
    para_idle_task[1]=0;
    para_idle_task[2]=0;
    para_idle_task[3]=2; // xi=2 means idle task
    ...
    #if ( INCLUDE_xTaskGetIdleTaskHandle == 1 )
    {
        ...
        xReturn = xTaskCreate( prvIdleTask, "IDLE", tskIDLE_STACK_SIZE, para_idle_task, ( tskIDLE_PRIORITY | portPRIVILEGE_BIT ), &xIdleTaskHandle ); /*lint !e961 MISRA exception, justified as it is not a redundant explicit cast to all supported compilers. */
    }
    #else
    {
        /* Create the idle task without storing its handle. */
        xReturn = xTaskCreate( prvIdleTask, "IDLE", tskIDLE_STACK_SIZE, para_idle_task, ( tskIDLE_PRIORITY | portPRIVILEGE_BIT ), NULL );  /*lint !e961 MISRA exception, justified as it is not a redundant explicit cast to all supported compilers. */
    }
    ...
    void delete_all_low_tasks_from(List_t * pxList){
                        ...
                        while(it<=pxList->uxNumberOfItems)
                        {
                            ...
                            for( pxIterator = ( ListItem_t * ) &( pxList->xListEnd ); it <= pxList->uxNumberOfItems ; pxIterator = pxIterator->pxNext ) /*lint !e826 !e740 The mini list structure is used as the list end to save RAM.  This is checked and valid. */
                            {
                                ...
                                {
                                    pxTCB=pxIterator->pvOwner;
                                    if(pxTCB->xi==0) // if its a low critical task
                                    {
                                        number_of_items=uxListRemove( &( pxTCB->xGenericListItem ) ); // delete it
                                        break; // start for next one
                                    }
                                }
                                it++;
                            } // end of for
        ...
        if(runtime >= critical_time_instance && mode==low && pxCurrentTCB->xi!=2)
            {
                ...
                set_deadlines_of_high_tasks_to_real_onces();
            }
#endif
    };
/*----------------------------------------------------------*/
...
static portTASK_FUNCTION( prvIdleTask, pvParameters )
{
    ...
    {
#if ( configUSE_EDFVD_SCHEDULER == 1 )
  listSET_LIST_ITEM_VALUE( &( ( pxCurrentTCB )->xGenericListItem ), ( pxCurrentTCB )->Deadline); // renew it's deadline
